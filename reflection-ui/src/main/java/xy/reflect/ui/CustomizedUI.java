...
public class CustomizedUI extends ReflectionUI {
	...
	/**
	 * Removes from the cache the {@link ITypeInfo} instances that were generated by
	 * the customization process. It forces the regeneration of these instances.
	 */
	public void clearCustomizationsCache() {
		getTypeCache().keySet().stream().filter(key -> (key instanceof ITypeInfo))
				.forEach(key -> getTypeCache().remove(key));
	}
