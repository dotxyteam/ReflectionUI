...
public class ResourcePath {
	...
	protected ResourcePath chosenAlternative;
	...
	public ResourcePath(String specification) {
		...
		if (specification.startsWith(ResourcePath.CLASSPATH_RESOURCE_PREFIX)) {
			...
			setFile(new File(specification));
		}
	}

	public String getSpecification() {
		if (chosenAlternative != null) {
			return chosenAlternative.getSpecification();
		}
	...
	public ResourcePath getChosenAlternative() {
		List<ResourcePath> alternatives = getAlternativeOptions();
		if (!alternatives.contains(chosenAlternative)) {
			if (alternatives.size() > 0) {
				chosenAlternative = alternatives.get(0);
			} else {
	...
	public List<ResourcePath> getAlternativeOptions() {
		...
		if (pathKind == PathKind.CLASS_PATH_RESOURCE) {
			result.add(new ResourcePath(ResourcePath.CLASSPATH_RESOURCE_PREFIX + path));
		}else{
			if (path.trim().length() > 0) {
				File file = new File(path);
				{
					...
					while (true) {
						...
						if (resourceURL != null) {
							result.add(
									new ResourcePath(ResourcePath.CLASSPATH_RESOURCE_PREFIX + candidateResourcePath));
						}
					}
				}
				{
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((chosenAlternative == null) ? 0 : chosenAlternative.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (chosenAlternative == null) {
			if (other.chosenAlternative != null)
				return false;
		} else if (!chosenAlternative.equals(other.chosenAlternative))
			return false;
		if (path == null) {
			if (other.path != null)
				return false;
