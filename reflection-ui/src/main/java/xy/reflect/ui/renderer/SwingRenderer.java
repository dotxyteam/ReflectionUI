...
public class SwingRenderer {
	...
	public List<JButton> createDialogOkCancelButtons(
		...
		final JButton okButton = new JButton(
				reflectionUI.prepareUIString((okCaption != null) ? okCaption
						: "OK"));
		result.add(okButton);
	...
	public Component createMultipleInfoCategoriesComponent(
		...
		for (final InfoCategory category : allCategories) {
			...
			tabbedPane.addTab(
					reflectionUI.prepareUIString(category.getCaption()), tab);
			...
			if (tabIndex < (tabCount - 1)) {
				JButton nextCategoryButton = new JButton(
						reflectionUI.prepareUIString(">"));
	...
	public Component createOnlineHelpControl(String onlineHelp) {
		...
		SwingRendererUtils.setMultilineToolTipText(result,
				reflectionUI.prepareUIString(onlineHelp));
			...
			if (settings.allReadOnly()) {
				if (!method.isReadOnly()) {
					if (methodControl instanceof JComponent) {
						SwingRendererUtils.disableComponentTree(
		...
		return new FieldInfoProxy(field) {
			...
			public void setValue(Object object, Object newValue) {
				ModificationStack stack = getModificationStackByForm()
						.get(form);
				stack.apply(new SetFieldValueModification(reflectionUI, object,
	...
	public ITypeInfo openConcreteClassSelectionDialog(
		...
		try {
			return reflectionUI.getTypeInfo(new JavaTypeInfoSource(Class
					.forName(className)));
	...
	public void openErrorDialog(Component activatorComponent, String title,
		...
		final JButton deatilsButton = new JButton(
				reflectionUI.prepareUIString("Details"));
	...
	public void openMethodReturnValueWindow(Component activatorComponent,
		...
		if (returnValue == null) {
			...
			openValueFrame(returnValue, IInfoCollectionSettings.DEFAULT,
					reflectionUI.getMethodTitle(object, method, returnValue,
							"Execution Result"));
		}
	}
	...
	public boolean openMethoExecutionSettingDialog(
		...
		JPanel methodForm = createObjectForm(new MethodParametersAsTypeInfo(
				reflectionUI, method).getPrecomputedTypeInfoInstanceWrapper(
		...
		JButton invokeButton = new JButton(method.getCaption());
		{
			invokeButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					}, reflectionUI.getMethodTitle(object, method, null,
							"Execution"));
					if (shouldDisplayReturnValue) {
		...
		methodDialogArray[0] = createDialog(activatorComponent, methodForm,
				reflectionUI.getMethodTitle(object, method, null, "Setting"),
	...
	public void updateFieldControlLayout(
		...
		if (!fieldControlHasCaption) {
			JLabel captionControl = new JLabel(
					reflectionUI.prepareUIString(field.getCaption() + ": "));
			GridBagConstraints layoutConstraints = new GridBagConstraints();
		...
		public void refreshUI() {
			if (fieldControl == null) {
				...
				if (!(((fieldControl instanceof IFieldControl) && ((IFieldControl) fieldControl)
						.refreshUI()))) {
					boolean hadFocus = SwingRendererUtils
