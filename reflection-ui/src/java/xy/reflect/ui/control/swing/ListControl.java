...
public class ListControl extends JPanel implements IFieldControl {
	...
	protected void updateToolbar() {
		...
		if (!UpdateListValueModification.isContainingListGetOnly(getRootListItemPosition())) {
	...
	protected boolean anySelectionItemContainingListGetOnly() {
		...
		for (AutoFieldValueUpdatingItemPosition selectionItem : selection) {
			if (UpdateListValueModification.isContainingListGetOnly(selectionItem)) {
				result = true;
				break;
			}
		}
			...
			public boolean isValid() {
				...
				if (newItemPosition != null) {
					if (newItemPosition.getContainingListType().canAdd()) {
						if (!UpdateListValueModification.isContainingListGetOnly(newItemPosition)) {
	...
	protected AbstractStandardListAction createAddChildAction() {
		return new AbstractStandardListAction() {
			...
			public boolean isValid() {
				...
				if (UpdateListValueModification.isContainingListGetOnly(subItemPosition)) {
					return false;
				}
	...
	protected AbstractAction createDynamicPropertyHook(final AbstractListProperty dynamicProperty) {
		return new AbstractStandardListAction() {
			...
			protected boolean perform(List<AutoFieldValueUpdatingItemPosition>[] toPostSelectHolder) {
				...
				boolean childValueCalculated = dynamicProperty.isValueDetached();
				return ReflectionUIUtils.integrateSubModifications(parentModifStack, childModifStack,
						childModifAccepted, childValueCalculated, commitModif, childModifTarget, null);
			}
	...
	protected AbstractStandardListAction createOpenItemAction() {
		return new AbstractStandardListAction() {
			...
			protected boolean perform(List<AutoFieldValueUpdatingItemPosition>[] toPostSelectHolder) {
				...
				IModification commitModif;
				if (UpdateListValueModification.isContainingListGetOnly(itemPosition)) {
					commitModif = null;
				} else {
				...
				boolean childValueCalculated = itemPosition.getContainingListField().isValueDetached();
				return ReflectionUIUtils.integrateSubModifications(parentModifStack, childModifStack,
						childModifAccepted, childValueCalculated, commitModif, childModifTarget, null);
	...
	protected void updateDetailsArea() {
		...
		detailsControl = swingRenderer.createFieldControl(null, new FieldInfoProxy(IFieldInfo.NULL_FIELD_INFO) {
			...
			public boolean isGetOnly() {
				return UpdateListValueModification.isContainingListGetOnly(detailsControlItemPosition);
			}
	...
	protected ObjectDialogBuilder getDetailsDialogBuilder(AutoFieldValueUpdatingItemPosition itemPosition) {
		...
		dialogBuilder.setGetOnly(UpdateListValueModification.isContainingListGetOnly(itemPosition));
